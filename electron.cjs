...
function startMongoDB() {
  const mongoPath = path.join(__dirname, 'mongodb-binaries', 'mongod.exe');
...
function startFlaskServer() {
  const flaskExePath = path.join(__dirname, 'flask_server', 'flask_server.exe');
  ...
  return fs.mkdir(userUploads, { recursive: true }).then(() => {
    return new Promise((resolve, reject) => {
